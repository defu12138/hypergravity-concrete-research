...
KEY_FILES = [
    ...
    PROJECT_ROOT / "reports" / "research_results_file_summary.xlsx",
]

TRACE_FILES = [
    PROJECT_ROOT / "data" / "processed" / "papers_master.csv",
    PROJECT_ROOT / "data" / "processed" / "direction_score.csv",
    PROJECT_ROOT / "data" / "processed" / "reading_list_by_direction.csv",
    PROJECT_ROOT / "data" / "processed" / "top_cited_papers.csv",
]
...
def status_line(path: Path) -> str:
    if path.exists():
        return f"- `{rel(path)}`：已存在"
    return f"- `{rel(path)}`：缺失"
...
def discover_recent_project_outputs() -> list[Path]:
    ...
    for folder_name in ["reports", "data/processed"]:
        ...
        for path in folder.iterdir():
            ...
            if any(keyword.lower() in lowered for keyword in FUTURE_WORK_KEYWORDS):
...
def build_prompt() -> str:
    missing = [path for path in KEY_FILES if not path.exists()]
    recent_outputs = discover_recent_project_outputs()
    missing_text = "\n".join(f"- `{rel(path)}`" for path in missing) if missing else "- 当前关键文件均存在。"
    recent_text = "\n".join(f"- `{rel(path)}`" for path in recent_outputs) if recent_outputs else "- 暂无额外后续产出记录。"
    updated_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    return f"""# NEXT_SESSION_PROMPT
...
{chr(10).join(status_line(path) for path in KEY_FILES)}
...
{chr(10).join(status_line(path) for path in TRACE_FILES)}
...
def main() -> int:
    OUTPUT_PATH.write_text(build_prompt(), encoding="utf-8")
